defmodule Oasis.Gen.Plug.TestBearerAuth do
  ...
  def handle_errors(conn, %{kind: _kind, reason: %Oasis.InvalidRequest{message: message} = reason, stack: _stack}) do
  ...
  def handle_errors(conn, %{kind: _kind, reason: %Oasis.InvalidTokenRequest{message: message} = reason, stack: _stack}) do
